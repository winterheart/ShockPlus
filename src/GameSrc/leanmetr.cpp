...
void lean_icon(LGPoint *pos, grs_bitmap **icon, int *inum) {
    ...
    if (!global_fullmap->cyber) {
        ...
        EDMS_lean_o_meter(PLAYER_PHYSICS, (fix &)ln, (fix &)crouch);
    ...
    *inum = posture * BMAPS_PER_POSTURE + ((100 - leanx) * BMAPS_PER_POSTURE / 201);
    ...
    FrameDesc *f = static_cast<FrameDesc *>(RefGet(MKREF(lean_bmap_res, *inum)));
    if (f != NULL) {
        f->bm.bits = (uchar *)(f + 1);
        *icon = &(f->bm);
    } else
...
void update_lean_meter(uchar force) {
    ...
    if (shield) {
        ...
        // Get a pointer to the corresponding lean bitmap.
	FrameDesc *f = static_cast<FrameDesc *>(RefGet(MKREF(shield_bmap_res, inum)));
	if (f != NULL) {
