...
uchar cutscene_mouse_handler(uiEvent *ev, LGRegion *r, intptr_t user_data) { return TRUE; }
...
void cutscene_exit(void) {
    ...
    if (movie_bitmap.bits != NULL) {
        free(movie_bitmap.bits);
...
short play_cutscene(int id, bool show_credits) {
    ...
    *GerSubtitle = 0;
    ...
    amovie = static_cast<Afile *>(malloc(sizeof(Afile)));
