...
void draw_weapons_list(inv_display *dp) {
    ...
    int i, s;
    short y;
    gr_set_font(static_cast<grs_font *>(ResLock(WEAPONS_FONT)));
...
static void generic_draw_list(inv_display *dp) {
    uchar newpage = inv_last_page != inventory_page;
    gr_set_font(static_cast<grs_font *>(ResLock(ITEM_FONT)));
...
void push_live_grenade_cursor(ObjID obj) {
    ...
    gr_set_font(static_cast<grs_font *>(ResGet(ITEM_FONT)));
    gr_clear(0);
#ifdef SVGA_SUPPORT
    if (convert_use_mode > 0) {
...
LGRegion *create_invent_region(LGRegion *root, LGRegion **pbuttons, LGRegion **pinvent) {
    ...
    if (!done_init) {
        ...
        f = static_cast<FrameDesc *>(RefLock(REF_IMG_bmBlankInventoryPanel));
        inv_backgnd = f->bm;
...
inv_display inv_display_list[] = {
    ...
    {2, 0,
     ...
     inv_select_general,
     general_use_func,
     static_cast<int32_t>(GENERAL_CLASSES),
     general_add_func,
     general_drop_func,
     0,
     NULL,
