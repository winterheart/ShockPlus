...
errtype difficulty_draw(uchar full) {
    ...
    if (full) {
        draw_raw_resource_bm(REF_IMG_bmDifficultyScreen, 0, 0);
        status_bio_draw();
    }
...
errtype journey_draw(char part) {
    ...
    if (part == 0)
        draw_raw_resource_bm(REF_IMG_bmJourneyOnwards, 0, 0);
...
void PrintWinStats(void) {
    ...
    gr_set_font(static_cast<grs_font *>(ResLock(RES_coloraliasedFont)));
...
errtype draw_sg_slot(int slot_num) {
    ...
    gr_set_font(static_cast<grs_font *>(ResLock(RES_smallTechFont)));
...
errtype journey_continue_func(uchar draw_stuff) {
    ...
    if (save_game_exists) {
        ...
        if (RefIndexValid(rt, i)) {
            FrameDesc *f = static_cast<FrameDesc *>(RefLock(rid));
            grs_bitmap bm = f->bm;
...
void splash_draw(bool show_splash) {
    ...
    hotKeyDispatcher.setContext(ShockPlus::Contexts::SETUP_CONTEXT);
