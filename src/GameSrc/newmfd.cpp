...
void screen_init_mfd(uchar fullscrn) {
    ...
    if (!done_init) {
        ...
        f = static_cast<FrameDesc *>(RefLock(REF_IMG_bmBlankMFD));
        mfd_background = f->bm;
...
uchar mfd_button_callback(uiEvent *e, LGRegion *r, intptr_t udata) {
    ...
    if (global_fullmap->cyber) {
        ...
        result = div((int)(e->pos.y - MFD_BTTN_Y), (int)(MFD_BTTN_SZ + MFD_BTTN_BLNK));
...
LGPoint mfd_full_draw_string(char *s, short x, short y, long c, int font, uchar DrawString, uchar transp) {
    ...
    short border = 0;
    grs_font *thefont = static_cast<grs_font *>(ResLock(font));
