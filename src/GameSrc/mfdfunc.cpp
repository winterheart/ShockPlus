...
void mfd_weapon_draw_ammo_buttons(int num_ammo_buttons, int ammo_subclass, ubyte *ammo_types, ubyte curr_ammo_type,
    ...
    if (ammo_count == 0) {
        ...
        gr_set_font(static_cast<grs_font *>(ResGet(RES_mediumLEDFont)));
        ...
        gr_set_font(static_cast<grs_font *>(ResGet(MFD_FONT)));
...
void mfd_elevator_expose(MFD *mfd, ubyte control) {
    ...
    if (elev_data->mfd_last[mfd->id].currlev == 0xF)
        ...
        if (full || elev_data->mfd_last[mfd->id].currlev != elev_data->stat.currlev) {
            ...
            gr_set_font(static_cast<grs_font *>(ResLock(ELEV_STATUS_FONT)));
...
void mfd_keypad_expose(MFD *mfd, ubyte control) {
    ...
    if ((keypad_data->special > 0) && (keypad_data->curr_digit > 0))
        ...
        if (full || (keypad_data->last_digit != keypad_data->curr_digit)) {
            ...
            gr_set_font(static_cast<grs_font *>(ResLock(KEYPAD_STATUS_FONT)));
