...
void init_popups(void) {
    for (int i = 0; i < NUM_POPUPS; i++) {
        ...
        FrameDesc *f = static_cast<FrameDesc *>(RefGet(id));
...
void make_popup_cursor(LGCursor *c, grs_bitmap *bm, char *s, uint tmplt, uchar allocate, LGPoint offset) {
    ...
    *bm = *pbm;
    ...
    gr_clear(0);
    ss_bitmap(pbm, 0, 0);
    gr_set_font(static_cast<grs_font *>(ResLock(RES_tinyTechFont)));
...
void make_email_cursor(LGCursor *c, grs_bitmap *bm, uchar page, bool init) {
    ...
    gr_font_char_size(static_cast<grs_font *>(ResGet(EMAIL_CURS_FONT)), 'X', &w, &h);
    h += 2;
    w = EMAIL_CURS_WID;
#ifdef SVGA_SUPPORT
    ss_point_convert(&w, &h, FALSE);
    ...
    gr_push_canvas(&gc);
    gr_clear(0);
    gr_set_font(static_cast<grs_font *>(ResLock(EMAIL_CURS_FONT)));
