...
void palette_advance_all_fx(long timestamp) {
    ...
    if (timestamps_per_step <= 0)
        return; // dont do a div by 0
...
void palette_advance_effect(byte id, int steps) {
    ...
    if (Palette_Effects_Table[id].mode == STEADY)
        steps = 1;
    ...
    if (Palette_Effects_Table[id].curr_dstep > Palette_Effects_Table[id].dsteps) {
        Palette_Effects_Table[id].curr_dstep -= Palette_Effects_Table[id].dsteps;

        add_to_color++;
    }

    if (Palette_Effects_Table[id].effect != SHIFT)
        add_to_color %= er;
    ...
    case SHIFT:
        ...
        if ((Palette_Effects_Table[id].curr_stage == -1) && (&grd_pal[e1 * 3] != &t[e1 * 3])) {
        ...
        if (Palette_Effects_Table[id].curr_stage >= Palette_Effects_Table[id].stages) {
    ...
    case CYCLE:
        ...
        if (m < 0)
            break; // Rare initial case w/fast frame rate
    ...
    case CBANK:
        ...
        if (num_installed_shifts == 0)
            break;
        else if (num_installed_shifts != 1)
            palette_swap_shadow((int)e1, (int)er, (int)a);
        else {
            for (i = 0; i < Palette_Effects_Table_Size; i++)
                if (Palette_Effects_Table[i].effect == SHIFT)
                    break;
            ...
            if ((s1 >= e1) && (sn <= en))
                palette_swap_shadow((int)s1, (int)sr, (int)a);
            else if (s1 > e1)
                palette_swap_shadow((int)s1, (int)(en - s1 + 1), (int)a);
            else if (sn < en)
                palette_swap_shadow((int)e1, (int)(sn - e1 + 1), (int)a);
            else
                palette_swap_shadow((int)e1, (int)er, (int)a);
        }
        break;
...
byte palette_install_effect(PAL_TYPE type, PAL_MODE mode, short b1, short b2, short b3, short b4, uchar *ptr1,
                            uchar *ptr2) {
    byte i;
    ...
    for (i = 0; i < Palette_Effects_Table_Size; i++)
        if (Palette_Effects_Table[i].status == EMPTY)
            break;

    if (i == Palette_Effects_Table_Size)
...
errtype palette_freeze_effect(byte id) {
    if (Palette_Effects_Table[id].status == EMPTY)
        return ERR_RANGE;
    if (Palette_Effects_Table[id].status == FROZEN)
        return ERR_NOEFFECT;
...
PAL_STATUS palette_query_effect(byte id) { return Palette_Effects_Table[id].status; }
...
void palette_set_rate(short ts) { timestamps_per_step = ts; }

void palette_initialize(short table_size) {
    int i;

    Palette_Effects_Table_Size = table_size;

    // Malloc the table

    Palette_Effects_Table = (PAL_TABLE_ENTRY *)malloc((int)table_size * sizeof(PAL_TABLE_ENTRY));
    //¥¥¥No error check here
...
void palette_init_smap(short first, short last, const uchar *from, const uchar *to, short num_steps) {
    int i, j;
    fix x0, x1, x2, y;
