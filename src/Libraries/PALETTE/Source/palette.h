...
// Different palette f(x) types:
typedef enum {
    SHIFT,  // shift
    CYCLE,  // single color cycle
    CBANK   // color banking
} PAL_TYPE;

// Status of loaded palette f(x)'s
typedef enum {
    EMPTY,
    ACTIVE,
    DELAYED,
...
// Different cycling/banking modes
typedef enum {
    REAL_TIME,
    STEADY
} PAL_MODE;
...
extern void palette_init_smap(short first, short last, const uchar *from, const uchar *to, short num_steps);
