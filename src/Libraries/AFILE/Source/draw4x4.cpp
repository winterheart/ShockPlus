...
class BitstreamInfo {
  ...
  public:
    ...
    inline void skip(int nbits) { count -= nbits; }
...
//
static uint8_t *Draw4x4_InternalBeta(uint32_t *xtab, int framesize, uint8_t *bits, int row, uint8_t *mask_stream) {
    ...
    while (bits < end) {
        ...
        case 0: {
            ...
            uint8_t ctab[2] = {uint8_t(xparam & 0xff), uint8_t((xparam & 0xff00) >> 8)};
            tbits = bits;
            for (i = 0; i < 4; ++i) {
                tbits[0] = ctab[0];
                tbits[1] = ctab[1];
        ...
        case 2: {
            ...
            if (ctab[0] != 0) {
                ...
                for (i = 0; i < 4; ++i) {
                    if (mask & 0x03)
                        tbits[0] = ctab[mask & 0x03];
                    if (mask & 0x0c)
                        tbits[1] = ctab[(mask & 0x0c) >> 2];
                    if (mask & 0x30)
                        tbits[2] = ctab[(mask & 0x30) >> 4];
                    if (mask & 0xc0)
                        tbits[3] = ctab[(mask & 0xc0) >> 6];
                    mask >>= 8;
                    tbits += row;
                }
            }
        ...
        case 3: {
            ...
            if (ctab[0] != 0) {
                ...
                for (i = 0; i < 4; ++i) {
                    if (mask & 0x007)
                        tbits[0] = ctab[mask & 0x007];
                    if (mask & 0x038)
                        tbits[1] = ctab[(mask & 0x038) >> 3];
                    if (mask & 0x1c0)
                        tbits[2] = ctab[(mask & 0x1c0) >> 6];
                    if (mask & 0xe00)
                        tbits[3] = ctab[(mask & 0xe00) >> 9];
                    mask >>= 12;
                    tbits += row;
                }
            }
        ...
        case 4: {
            ...
            if (ctab[0] != 0) {
                ...
                for (i = 0; i < 4; ++i) {
                    if (mask & 0x000f)
                        tbits[0] = ctab[mask & 0x000f];
                    if (mask & 0x00f0)
                        tbits[1] = ctab[(mask & 0x00f0) >> 4];
                    if (mask & 0x0f00)
                        tbits[2] = ctab[(mask & 0x0f00) >> 8];
                    if (mask & 0xf000)
                        tbits[3] = ctab[(mask & 0xf000) >> 12];
                    mask >>= 16;
                    tbits += row;
                }
            }
