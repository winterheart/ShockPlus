...
MemStack *temp_mem_get_stack(void) { return temp_mem_stack; }
...
void *temp_realloc(void *p, long n) { return MemStackRealloc(temp_mem_stack, p, n); }

/* free temporary buffer pointed to by p. */
int temp_free(void *p) { return MemStackFree(temp_mem_stack, p) == FALSE; }
...
int temp_spew_mem_init(MemStack *ms, char *file, int line) {
    int r;
    r = temp_mem_init(ms);
    Spew(DSRC_LG_Tempmem, ("TempMemInit: stack: %p rval: %d (file: %s line: %d)\n", temp_mem_stack, r, file, line));
    return r;
}
...
int temp_spew_mem_uninit(char *file, int line) {
    ...
    r = temp_mem_uninit();
    Spew(DSRC_LG_Tempmem, ("TempMemUninit rval: %d (file: %s line: %s)\n", r, file, line));
    return r;
}
...
void *temp_spew_malloc(long size, char *file, int line) {
    void *p;
    p = temp_malloc(size);
    Spew(DSRC_LG_Tempmem, ("TempMalloc:  p: 0x%x  size: %d  (file: %s line: %d)\n", p, size, file, line));
    return p;
}
...
int temp_spew_free(void *ptr, char *file, int line) {
    int r;
    r = temp_free(ptr);
    Spew(DSRC_LG_Tempmem, ("TempFree:    p: 0x%x  (file: %s line: %d)\n", ptr, file, line));
    return r;
}
#endif /* DBG_ON */
