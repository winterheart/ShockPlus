...
#define poly_do_tw(_tf, _ti, _d, _frac, _t0, _dt, _wf, _wi, _w0, _dw) \
    do {                                                              \
        if (fix_abs(_tf - _ti) < FIX_UNIT / 16) {                     \
            _t0 = fix_mul(_ti, _w0);                                  \
            _dt = fix_mul(_ti, _dw);                                  \
            if (_ti - fix_floor(_ti) < FIX_UNIT / 2)                  \
                _t0++, _dt++;                                         \
            else                                                      \
                _t0--, _dt--;                                         \
        } else {                                                      \
...
void gri_x_edge(grs_tmap_loop_info *info, grs_vertex **p, grs_vertex **p_prev, int side) {
    grs_tmap_edge *edge;
    fix d, dy;
...
void gri_ix_edge(grs_tmap_loop_info *info, grs_vertex **p, grs_vertex **p_prev, int side) {
    grs_tmap_edge *edge;
    fix d, frac;
    grs_vertex *prev, *next;
...
void gri_rgbx_edge(grs_tmap_loop_info *info, grs_vertex **p, grs_vertex **p_prev, int side) {
    grs_tmap_edge *edge;
    fix d, frac;
    grs_vertex *prev, *next;
...
void gri_uvx_edge(grs_tmap_loop_info *info, grs_vertex **p, grs_vertex **p_prev, int side) {
    grs_tmap_edge *edge;
    fix d, frac;
    grs_vertex *prev, *next;
...
void gri_uvix_edge(grs_tmap_loop_info *info, grs_vertex **p, grs_vertex **p_prev, int side) {
    grs_tmap_edge *edge;
    fix d, frac;
    grs_vertex *prev, *next;
...
void gri_uvwx_edge(grs_tmap_loop_info *info, grs_vertex **p, grs_vertex **p_prev, int side) {
    grs_tmap_edge *edge;
    fix d, frac;
    grs_vertex *prev, *next;
    ...
    prev = *p_prev, next = *p;
    poly_do_x(next, prev, d, frac, edge->x, edge->dx, edge->y);
    poly_do_tw(next->u, prev->u, d, frac, edge->u, edge->du, next->w, prev->w, info->w, info->dw);
    poly_do_tw(next->v, prev->v, d, frac, edge->v, edge->dv, next->w, prev->w, info->w, info->dw);
}

void gri_uviwx_edge(grs_tmap_loop_info *info, grs_vertex **p, grs_vertex **p_prev, int side) {
    grs_tmap_edge *edge;
    fix d, frac;
    grs_vertex *prev, *next;
    ...
    poly_do_tw(next->u, prev->u, d, frac, edge->u, edge->du, next->w, prev->w, info->w, info->dw);
    poly_do_tw(next->v, prev->v, d, frac, edge->v, edge->dv, next->w, prev->w, info->w, info->dw);
    poly_do_tw(next->i, prev->i, d, frac, edge->i, edge->di, next->w, prev->w, info->w, info->dw);
}

void gri_uvwy_edge(grs_tmap_loop_info *info, grs_vertex **p, grs_vertex **p_prev, int side) {
    grs_tmap_edge *edge;
    fix d, frac;
    grs_vertex *prev, *next;
...
void gri_uviwy_edge(grs_tmap_loop_info *info, grs_vertex **p, grs_vertex **p_prev, int side) {
    grs_tmap_edge *edge;
    fix d, frac;
    grs_vertex *prev, *next;
