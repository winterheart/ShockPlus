...
int gri_per_umap_setup(int n, grs_vertex **vplist, grs_per_setup *ps) {
    ...
    if (delta_min < y_max - y_min)
        delta_min = y_max - y_min;
        ...
        for (v1 = vplist[i = 0]; i < n; v1 = vplist[++i]) {
            for (v2 = vplist[j = i + 1]; j < n; v2 = vplist[++j]) {
                for (v3 = vplist[k = j + 1]; k < n; v3 = vplist[++k]) {
                    minsd = fix_abs(v1->x - v2->x) + fix_abs(v1->y - v2->y);
                    sd = fix_abs(v3->x - v2->x) + fix_abs(v3->y - v2->y);
                    if (sd < minsd)
                        minsd = sd;
                    sd = fix_abs(v1->x - v3->x) + fix_abs(v1->y - v3->y);
                    if (sd < minsd)
                        minsd = sd;
                    if (minsd > maxsd) {
                        maxsd = minsd;
    ...
    **************************************************************************/
    ...
    ps->gamma_v = fix_mul_16_32_20(fix_mul_3_16_16(c0, wvz - (fix_mul(wvx, aos) + fix_mul(wvy, bos))), scale);
    ps->alpha_u = fix_mul_3_16_20(c0, wux);
    ps->beta_u = fix_mul_3_16_20(c0, wuy);
    ps->gamma_u = fix_mul_16_32_20(fix_mul_3_16_16(c0, wuz - (fix_mul(wux, aos) + fix_mul(wuy, bos))), scale);
