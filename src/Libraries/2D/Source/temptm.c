...
void temp_point(short x, short y) { gr_fill_pixel(grd_gc.fcolor, x, y); }

void temp_rsd8_ubitmap(grs_bitmap *bm, int x, int y) {
    ((void (*)(grs_bitmap * _bm, int _x, int _y)) grd_function_table[GRC_BITMAP + BMT_RSD8 * GRD_FUNCS])(bm, x, y);
}
...
void temp_tluc8_ubitmap(grs_bitmap *bm, int x, int y) {
    ((void (*)(grs_bitmap * _bm, int _x, int _y)) grd_function_table[GRC_BITMAP + BMT_TLUC8 * GRD_FUNCS])(bm, x, y);
}
...
#define make_vertex(_vertex, _x, _y, _u, _v) \
    _vertex.x = fix_make(_x, 0), _vertex.y = fix_make(_y, 0), _vertex.u = _u, _vertex.v = _v
...
int gri_scale_clip(grs_vertex *v0, grs_vertex *v1) {
    ...
    if ((v0->x >= grd_fix_clip.right) || (v1->x <= grd_fix_clip.left) || (v0->y >= grd_fix_clip.bot) ||
        (v1->y <= grd_fix_clip.top))
        return CLIP_ALL;
