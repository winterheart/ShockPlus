...
int gr_alloc_ipal(void) {
    ...
    if (grd_ipal != NULL) {
        if ((err = gr_free_ipal()) < 0)
            return err;
    }

    if ((grd_ipal = (uchar *)malloc(32768)) == NULL)
        return RGB_OUT_OF_MEMORY; // was gr_malloc

    if ((err = gr_init_ipal()) < 0)
        return err;
    return RGB_OK;
}
...
int gr_init_ipal(void) {
    ...
    if ((data = (uchar *)malloc(3 * 256)) == NULL)
        return RGB_OUT_OF_MEMORY; // was gr_malloc
    ...
    if ((dist_buf = (ulong *)malloc(sizeof(ulong) * 32768)) == NULL)
        return RGB_OUT_OF_MEMORY; // was gr_malloc
...
static int _greenloop(int restart) {
    ...
    /* Basic loop down */
    for (g = here - 1, gxx = ginc - txsqr, gcdist = gdist = rdist - gxx, gcdp = gdp = rdp - gstride,
...
static int _blueloop(int restart) {
    ...
    for (b = here, bdist = gdist, bxx = binc, dp = gdp, rgbp = grgbp, lim = max; b <= lim;
