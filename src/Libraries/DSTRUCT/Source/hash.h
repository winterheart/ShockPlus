...
// insert an element into a hashtable, overwriting any element that is equal to it.
errtype hash_set(Hashtable *h, void *elem);

// REQUIRES there is no member of h equal to "elem". inserts "elem" into hashtable h. Faster than hash_set.
errtype hash_insert(Hashtable *h, void *elem);

// Looks up elem in the hashtable, sets *result to point to the element in h which is equal to elem.
// Or NULL if no such element exists.
// WARNING WARNING DANGER WILL ROBINSON. HEINOUS REP EXPOSURE. MODIFY **RESULT AT YOUR OWN PERIL
errtype hash_lookup(Hashtable *h, void *elem, void **result);

// Find and remove the element in h which is equal to elem, or do nothing if no such element exists.
errtype hash_delete(Hashtable *h, void *elem);

typedef uchar (*HashIterFunc)(void *elem, void *data);

// Applies ifunc(elem,data) to every element of h, one at a time, until ifunc returns true.
...
// Destroys hashtable h. Does not free h itself, but frees subordinate data structures.
