...
typedef struct _slist {
    struct _slist *psnext; // ptr to next node or NULL if last one real data follows, here
} slist;
...
#define slist_remove(psl, pslbefore) \
    { (pslbefore)->psnext = (psl)->psnext; }
...
#define slist_next(psl) (slist *)((psl)->psnext)

// Iterate across all items
#define forallinslist(listtype, pslh, psl) \
    for (psl = (listtype *)slist_head(pslh); psl != NULL; psl = (listtype *)slist_next(psl))
