                ...
                if (event[i]->status == 0xFF && event[i]->data[0] == 0x51) // tempo change
                {
                    ...
                    if ((event[i]->status & ~15) == 0xB0 && event[i]->data[0] == 0x07) {
                        ...
                        if (MusicDev && MusicDev->isOpen) {
                            ...
                            const int scaledVolume = ((int)p2 * (int)ShockPlus::Options::musicVolume) / 100;
...
void InitDecXMI(void) {
    ...
    int musicrate = 48000;
    switch (ShockPlus::Options::midiBackend) {
#ifdef USE_LIBADLMIDI
    case ShockPlus::Options::MIDI_ALMIDI: // adlmidi
    {
        INFO("Creating ADLMIDI device");
        musicdev = CreateMusicDevice(Music_AdlMidi);
    } break;
#endif
    case ShockPlus::Options::MIDI_NATIVE: // native midi
    {
        INFO("Creating native MIDI device");
        musicdev = CreateMusicDevice(Music_Native);
    } break;
#ifdef USE_FLUIDSYNTH
    case ShockPlus::Options::MIDI_FLUIDSYNTH: // fluidsynth
    {
    ...
    INFO("Opening MIDI device using output %d", ShockPlus::Options::midiOutput);
    if (musicdev && musicdev->init(musicdev, ShockPlus::Options::midiOutput, musicrate) != 0) {
    ...
    if (!musicdev) {
        ...
        if (musicdev) {
            musicdev->init(musicdev, ShockPlus::Options::midiOutput, musicrate);
        }
    }

    // force prefs to align with music device output
    if (musicdev) {
        ShockPlus::Options::midiOutput = musicdev->outputIndex;
    }
...
void ReloadDecXMI(void) {
...
#ifdef USE_LIBADLMIDI
        case Music_AdlMidi:
            deviceTypeMatch = (ShockPlus::Options::midiBackend == ShockPlus::Options::MIDI_ALMIDI);
            break;
#endif
        case Music_Native:
            deviceTypeMatch = (ShockPlus::Options::midiBackend == ShockPlus::Options::MIDI_NATIVE);
            break;
#ifdef USE_FLUIDSYNTH
        case Music_FluidSynth:
            deviceTypeMatch = (ShockPlus::Options::midiBackend == ShockPlus::Options::MIDI_FLUIDSYNTH);
            break;
#endif
        }
    }
    ...
    if (MusicDev && (!MusicDev->isOpen || !deviceTypeMatch || MusicDev->outputIndex != ShockPlus::Options::midiOutput)) {
        SDL_UnlockMutex(MyMutex);
...
void UpdateVolumeXMI(void) {
    ...
    INFO("UpdateVolumeXMI(): Global music volume change to %d percent", ShockPlus::Options::musicVolume);
    ...
    if (MusicDev && MusicDev->isOpen) {
        ...
        for (int i = 0; i <= 15; ++i) {
            ...
            if (i != 9 && ChannelThread[i] == -1)
                continue;
            // scale new volume according to global music volume
            const int scaledVolume = ((int)SDL_AtomicGet(&DeviceChannelVolume[i]) * (int)ShockPlus::Options::musicVolume) / 100;
